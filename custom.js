...
function glenToDo() {
  ...
  //Wipe out all the old cards on refresh
$('.card').remove();
    ...
    for (var x = 0, y = json.values.length; x < y; x++) {
      ...
      for (var i = 0; i < y; i++) {
        ...
        if (valList != '' && valList != null && valList != ' ') {
          ...
          if (i == 1) {
            $('.card-columns').append('<div class="card"><ul></ul></div>');

            //Make the first value an H2 header
            $('.card:nth-of-type(' + x + ') ul').append('<h2>' + valList + '</h2>');
          } else {

            //Make all values other than the first in a column basic list objects
            $('.card:nth-of-type(' + x + ') ul').append('<li>' + valList + '</li>');
          }
        }
      }
    }
